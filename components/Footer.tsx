...
/* ═══════════════════════════════════════════════════
   D'ORGANICA FOOTER — Premium, Warm, Authoritative
   ═══════════════════════════════════════════════════ */
...
const FOOTER_TRAINING = [
  ...
  { label: "In-Person Training", href: "/training#inperson" },
      ...
      <div className="relative mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 lg:py-20">
        ...
        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <div className="sm:col-span-2 lg:col-span-2">
            ...
            <p className="mt-2 text-xs font-medium italic text-forest-400">
            ...
            <div className="mt-6 flex items-center gap-3">
              {SOCIAL_LINKS.map((link) => (
                <a
                  ...
                  className="flex h-10 w-10 items-center justify-center rounded-full bg-forest-800 text-forest-300 transition-all hover:bg-forest-700 hover:text-white"
          ...
          {/* Quick Links */}
          <div>
            <h3 className="mb-4 text-sm font-semibold uppercase tracking-wider text-forest-300">
              Quick Links
            </h3>
            <ul className="space-y-2.5">
              {NAV_ITEMS.map((item) => (
                <li key={item.label}>
                  <Link
                    href={item.href}
                    className="text-sm text-forest-400 transition-colors hover:text-white"
                  >
                    {item.label}
                  </Link>
                </li>
        ...
        <div className="mt-12 grid gap-4 border-t border-forest-800 pt-8 sm:grid-cols-3">
          <div className="flex items-start gap-3">
            <svg className="mt-0.5 h-4 w-4 shrink-0 text-forest-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
              <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" />
              <circle cx="12" cy="10" r="3" />
            </svg>
            <span className="text-sm text-forest-400">{BRAND.location}</span>
          </div>
          <div className="flex items-start gap-3">
            <svg className="mt-0.5 h-4 w-4 shrink-0 text-forest-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
          ...
          <div className="flex items-start gap-3">
            ...
            <a href={`mailto:${CONTACT.email}`} className="text-sm text-forest-400 hover:text-white transition-colors">{CONTACT.email}</a>
          </div>
        </div>
        ...
        <div className="mt-8 border-t border-forest-800 pt-8 text-center sm:flex sm:items-center sm:justify-between sm:text-left">
          ...
          <p className="mt-2 text-xs text-forest-600 sm:mt-0">
